...
public class TopLoop extends Loop implements Node {
    ...
    public TopLoop(String text, NodeList parent) {
        super(text);
        setParent(parent);
    }
    
    public TopLoop(JsonObject obj, NodeList parent) {
        super(obj);
        setParent(parent);
    }
...
//        text.setStyle("-fx-background-color: #eee; -fx-background-insets: 0; -fx-background-radius: 0;");
    ...
    public Node snapshot(NodeList snapshotParent) {
        TopLoop snapshot = new TopLoop(this.text, snapshotParent);
